...
class LongestCommonSubsequence:
    ...
    def solve(self):
        ...
        table_plot = ax.table(
            cellText=table.astype(str),
            # Initialize with zeros in string format
            rowLabels=list(self.str1),
            # Empty label at start for row labels
            colLabels=list(self.str2),
            # Empty label at start for column labels
            loc="center",
            cellLoc="center",
        )
        ...
        def update(frame):
            ...
            for x in range(self.m):
                for y in range(self.n):
                    ...
                    table_plot[x + 1, y].get_text().set_text(
                        str(self.dp[x][y]))

...
class KnapsackProblem:
    def __init__(self, weights, values, capacity):
        ...
        self.dp = [[0] * (self.capacity + 1) for _ in range(self.num_items + 1)]

    def solve(self):
        fig, ax = plt.subplots(figsize=(10, 6))  # Adjust window size for larger display
        
        # Create a table with headers for weights and values
        headers = [f'W={w}' for w in range(self.capacity + 1)]
        rows = [f'Item {i}\n(w={self.weights[i]}, v={self.values[i]})' for i in range(self.num_items)]
        
        # Initialize table data with headers intact
        table_data = [[''] * (self.capacity + 1) for _ in range(self.num_items)]
        ...
        def update(frame):
            ...
            for r in range(1, self.num_items + 1):  # Skip the first row (header)
                for c in range(self.capacity + 1):  # Skip the first column (header)
        ...
        frames = [(i, w) for i in range(1, self.num_items + 1) for w in range(self.capacity + 1)]
